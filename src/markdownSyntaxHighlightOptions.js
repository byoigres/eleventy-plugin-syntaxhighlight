...
module.exports = function (options = {}) {
  let plugin = function (str, language) {
    ...
    if (options.showLineNumbers) {
      lineNumbers = `<span aria-hidden="true" class="line-numbers-rows">${new Array(
        lines.length + 1
      ).join("<span></span>")}</span>`;
    }

    let classNames =
      `language-${language}` + (options.showLineNumbers ? " line-numbers" : "");
    //  the line-numbers CSS class shifts the lines right
    //  to make room for the line numbers

    let retStr =
      `<pre class="${classNames}" data-language="${language}">` +
      `<code class="language-${language}" data-language="${language}">${lines.join(
        "<br>"
      )}` +
      lineNumbers +
      `</code></pre>`;
